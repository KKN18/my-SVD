...
class MyStableVideoDiffusionPipeline(StableVideoDiffusionPipeline):
    ...
    def __call__(
        ...
        self.check_inputs(base_image, height, width)
        
        batch_size = 1
        device = self._execution_device
        
        base_image_embeddings = self._encode_image(base_image, device, num_videos_per_prompt, self.do_classifier_free_guidance)
        fps = fps - 1

        processed_base_image = self.image_processor.preprocess(base_image, height=height, width=width).to(device)
        noise = randn_tensor(processed_base_image.shape, generator=generator, device=device, dtype=processed_base_image.dtype)
        processed_base_image = processed_base_image + noise_aug_strength * noise

        base_image_latents = self._encode_vae_image(
            processed_base_image,
            device=device,
        ...
        for image in images:
            ...
            ele_latents = self._encode_vae_image(
                image,
                device=device,
                num_videos_per_prompt=num_videos_per_prompt,
                do_classifier_free_guidance=self.do_classifier_free_guidance,
            )
            ele_latents = ele_latents.to(torch.float32)
        ...
        for i in range(latents.size(1) - 1):
            ...
            random_latent = randn_tensor(prev_latent.shape, generator=generator, device=device, dtype=prev_latent.dtype)
            new_latents.append(random_latent)
        ...
        fix_indices = list(range(0, latents.size(1), 2))

        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                for idx in fix_indices:
                    fix_latent = fix_latents[:, idx//2]
                    rand_noise = torch.rand_like(fix_latent)
                    noisy_latents = self.scheduler.add_noise(fix_latent, rand_noise, t)
                    latents[:, idx] = noisy_latents
...
frames = pipe(images, decode_chunk_size=1, num_frames=7, generator=generator).frames[0]

save_path = f"/home/nas2_userG/junhahyung/kkn/my-svd/outputs/main_fix/time_step_0"
