        ...
        with precision_scope("cuda"):
            with model.ema_scope():
                ...
                def print_dict(x):
                    ...
                    for key, value in x.items():
                        ...
                        if isinstance(value, torch.Tensor):
                            # Print the key and the shape of the tensor
                ...
                if test_mode == True:
                    ...
                    for k in c_list[0]:
                        i=0
                        for ce, uce in zip(c_list, uc_list):
                            if not k == "crossattn" and not (k == "vector" and i!=0):
                                ce[k], uce[k] = map(
                                    lambda y: y[k][: math.prod(num_samples)].to("cuda"), (ce, uce)
                                )
                            i += 1
