...
class MyStableVideoDiffusionPipeline(StableVideoDiffusionPipeline):
    ...
    def __call__(
        ...
        for i in range(latents.size(1) - 1):
            ...
            random_latent = torch.rand_like(prev_latent)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                for idx in fix_indices:
                    ...
                    timestep_tensor = torch.FloatTensor([t]).to("cuda:0")
                    noisy_latents = self.scheduler.add_noise(fix_latent, rand_noise, timestep_tensor)
                    print(f'[idx: {idx}] noisy_latents: {noisy_latents.shape}')
